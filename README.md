# Smart Home (SmashCore Dashboard)

Фронтенд-панель (Dashboard) для работы с `SmashCore` – C++ бэкендом для управления умным домом.

## Запуск проекта

### Frontend

Фронтенд построен с использованием React и Vite. Для отправки запросов к SmashCore, он использует переменную окружения `VITE_API_URL` (по умолчанию `http://127.0.0.1:8080/api`).
...
В браузере откроется `http://localhost:5173`.
Для авторизации используйте ваш `X-API-Key` (Например, `admin123`).
